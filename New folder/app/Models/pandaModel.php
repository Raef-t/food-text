...
$images=[];
        foreach (array_slice($variety['images'], 1) as $Liist) {
            $images[] = $Liist[0];
        }
        return new self([
            'id' => $variety['id'],
            ...
            'image_url' => $variety['imageURL'] ?? null,
           "images_url"=> $images

        ]);
    }
    public static function fromRameszApiResponse(array $data)
    {
        $variety = isset($data['product_barcodes'][0]) ? $data['product_barcodes'][0] : [];
$images=[];
        foreach ($data['images'] as $Liist) {
            $images[] = $Liist;
        }
        return new self([
            'id' => $variety['id'],
            'name' => $data['h_name'],
            'description' => $variety['h_description'] ?? '',  // Safely access description
            'price' => $variety['price'] ?? '',              // Safely access price
            'category_id' => $data['category_id'] ?? null,   // Safely handle missing category_id
            'brand_id' => $data['brand_id'] ?? null,         // Safely handle missing brand_id
            'size' => $variety['size'] ?? '',                // Safely access size
