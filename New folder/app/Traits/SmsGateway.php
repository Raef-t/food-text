...
trait  SmsGateway
{
    ...
    public static function sendSms($receiver, $otp): string
    {
        // Get 4jawaly settings (you can replace this with your own configuration method)
//        $config = self::get_settings('4jawaly');
        $response = 'error';

        if (true) {
            $app_id = "81XkZw1dJHZ73OgmzAzSJlrz3X1RgcqqZnFktvIh";
            $app_sec = "qjTbwtlkUbUcBTxxnoRwa7kZ64AqTH62Z0atFUpLG8EStsfmKCXmvAQzPqIbkTCdwHrnyDRfkdDhr0RmMBrFHpiT2ITIHgmQV6eJ";
            $sender = "Shella food"; // Your registered sender name with 4jawaly

            $app_hash = base64_encode("{$app_id}:{$app_sec}");
            $message="ان هذه الرسالة مرسله لك من تطبيق شله المملوك لشركه بوابه التسهيل وكود ال otp الخاص بك هوا ".$otp;
            $payload = [
                "messages" => [
                    [
                        "text" => $message,
            ...
            try {
                ...
                $response_json = json_decode($responseBody, true);

                if ($status_code == 200) {
                    return json_encode( $response_json);
                    if (isset($response_json["messages"][0]["err_text"])) {
                        $response = $response_json["messages"][0]["err_text"];
                    } else {
                        $response = "success" ;
                    }
                } elseif ($status_code == 400) {
                    $response = $response_json["message"] ?? 'Bad Request';
                } elseif ($status_code == 422) {
                    $response = "نص الرسالة فارغ";
                } else {
                    $response = "محظور بواسطة كلاودفلير. Status code: {$status_code}";
                }
            } catch (Exception $e) {
