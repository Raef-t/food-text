...
class Helpers
{
    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en' , $temp_product=false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $categories = [];
            try{
                $categoriesIds=json_decode($item['category_ids']);
                ...
                $categoriesIds=json_decode($item['category_ids'],true);
                foreach ($categoriesIds as $value) {
    ...
    public static function store_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                unset($item['rating']);
                $data["category_idsss"] = json_decode($item["category_idsss"],true);
            ...
            try {
                ...
                if (!is_array($data["category_idsss"])) {
                    $data["category_idsss"] = json_decode($data["category_idsss"], true);
                }

            }catch (\Exception $e) {
                return response()->json(['status' => $data, 'message' => $e->getMessage()]);
            }
            $data["category_idsss_data"] = Category::whereIn('id',$data["category_idsss"])->get();
        ...
        if ($multi_data) {
            foreach ($data as $item) {
                ...
                if(isset($item->group_order)){
                    $item['collection_order']=$item->group_order->pluck("order_group_id")->toArray();

                }
                if($item->collection_order==1){
                    $item['collection_order']=1;
                }
                array_push($storage, $item);
            }
            $data = $storage;
    ...
    public static function order_details_data_formatting($data,$orderGroup=[])
    {
        $storage = [];
        foreach ($data as $item) {
        ...
        // Format items in $orderGroup
        foreach ($orderGroup as $group) {
            // Check if OrderGroupId and details exist before accessing them
            if (isset($group->OrderGroupId) && isset($group->OrderGroupId->details)) {
                foreach ($group->OrderGroupId->details as $item) {
                    $item['add_ons'] = json_decode($item['add_ons']);
                    $item['variation'] = json_decode($item['variation'], true);
                    $item['item_details'] = json_decode($item['item_details'], true);
    ...
    public static function order_details_data_formatting_e_user($data)
    {
        $storage = [];
        foreach ($data as $item) {
            $item['add_ons'] = json_decode($item['add_ons']);
