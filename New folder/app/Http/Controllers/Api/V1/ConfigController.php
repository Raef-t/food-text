...
//            'landing_page_settings_full_url'=> $data['web_app_landing_page_settings_storage']??'public',
            ...
            'price_after_limit'=>(float)(isset($settings['price_after_limit']) ? $settings['price_after_limit'] : 0),
             'limit_for_delevry'=>(float)(isset($settings['limit_for_delevry']) ? $settings['limit_for_delevry'] : 0),
             'stop_price'=>(float)(isset($settings['stop_price']) ? $settings['stop_price'] : 0),
    ...
    public function place_api_autocomplete(Request $request)
    {
        ...
        $response = Http::get('https://maps.googleapis.com/maps/api/place/autocomplete/json?input='.$request['search_text'].'&key='.$this->map_api_key.'&language=ar');
        return $response->json();
    }
    ...
    public function distance_api(Request $request)
    {
        ...
        $response = Http::get('https://maps.googleapis.com/maps/api/distancematrix/json?origins='.$request['origin_lat'].','.$request['origin_lng'].'&destinations='.$request['destination_lat'].','.$request['destination_lng'].'&key='.$this->map_api_key.'&mode=drive');
        return $response->json();
    }
    ...
    public function place_api_details(Request $request)
    {
        ...
        $response = Http::get('https://maps.googleapis.com/maps/api/place/details/json?placeid='.$request['placeid'].'&key='.$this->map_api_key.'&language=ar');
        return $response->json();
    }
    ...
    public function geocode_api(Request $request)
    {
        ...
        $response = Http::get('https://maps.googleapis.com/maps/api/geocode/json?latlng='.$request->lat.','.$request->lng.'&key='.$this->map_api_key.'&language=ar');
        return $response->json();
    }
    ...
    private function getDefaultPaymentMethods()
    {
        ...
        $methods = Setting::where('is_active',1)->whereIn('settings_type', ['payment_config'])->whereIn('key_name', ['ssl_commerz','paypal','stripe','razor_pay','senang_pay','paytabs','paystack','paymob_accept','paytm','flutterwave','liqpay','bkash','mercadopago','myfatoorah'])->get();
        ...
        foreach ($methods as $method) {
            ...
            if ($credentialsData['status'] == 1) {
                $data[] = [
                    ...
                    'gateway_title' => $additional_data?->gateway_title,
                    'gateway_image' => $additional_data?->gateway_image,
                    'gateway_image_full_url' => Helpers::get_full_url('payment_modules/',$additional_data?->gateway_image,$additional_data?->storage ?? 'public')
                ];
            }
        }
        return $data;
