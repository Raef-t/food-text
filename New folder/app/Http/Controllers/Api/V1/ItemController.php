...
$store=Store::find($request['store_id']);
        ...
        $items = ProductLogic::get_latest_products($zone_id, $request['limit'], $request['offset'],$store->is_subscription?$request['store_id']:3, $request['category_id'], $type, $min, $max, $product_id);
    ...
    public function get_popular_products(Request $request)
    {
        ...
        $limit = $request->get('limit', 10); // Default limit if not provided
        $offset = $request->get('offset', 0); // Default offset if not provided

        // Cache key based on zone_id, type, limit, and offset
        $cacheKey = "popular_products_{$zone_id}_{$type}_{$limit}_{$offset}_{$moduleId}";

        // Attempt to retrieve cached data
        $items = Cache::remember($cacheKey, 60 * 60, function () use ($zone_id, $limit, $offset, $type) {
            // Fetch products from the database if not in cache
            return ProductLogic::popular_products($zone_id, $limit, $offset, $type);
        });
    ...
    public function get_most_reviewed_products(Request $request)
    {
        ...
        // Retrieve parameters
        $type = $request->query('type', 'all');  // Default to 'all' if no type is provided
        $zone_id = $request->header('zoneId');
        $moduleId = $request->header('moduleId');
        $limit = $request->query('limit', 10);  // Default limit to 10
        $offset = $request->query('offset', 0); // Default offset to 0

        // Create a unique cache key based on zone_id, limit, offset, and type
        $cacheKey = "most_reviewed_products_{$zone_id}_{$limit}_{$offset}_{$type}_{$moduleId}";

       $items = Cache::remember($cacheKey, 300, function () use ($zone_id, $limit, $offset, $type) {
            // Assuming ProductLogic::most_reviewed_products is a custom method that retrieves the products
            return ProductLogic::most_reviewed_products($zone_id, $limit, $offset, $type);
        });
