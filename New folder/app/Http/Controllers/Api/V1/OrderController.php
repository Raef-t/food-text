...
use App\Jobs\CheckOrderAcceptance;
use App\Models\Cart;
use App\Models\Item;
use App\Models\Zone;
...
class OrderController extends Controller
{
    ...
    public function track_order(Request $request)
    {
        try {
            ...
            $order = Order::with(['store', 'store.store_sub', 'delivery_man.rating', 'parcel_category', 'refund', 'payments', "OrderRequests", 'OrderRequests.deliveryMan'])->withCount('details')->where('id', $request['order_id'])
            ...
            if ($order) {
                ...
                $time=$this->calculateDeliveryTime($order->store->latitude??0, $order->store->longitude??0, $order->delivery_address->latitude??0, $order->delivery_address->longitude??0,(int)explode('-', $order->store?->delivery_time)[0],(int)explode('-', $order->store?->delivery_time)[1] ,);
                $order['min_delivery_time'] = $time["min_delivery_time"];
                $order['max_delivery_time'] = $time["max_delivery_time"];
    ...
    function calculateDeliveryTime(
        ...
        $destinationLat,
        $destinationLng,
        $storeMinTime,  // Store minimum preparation time (in minutes)
        $storeMaxTime,  // Store maximum preparation time (in minutes)
        $mode = 'driving',
        $language = 'ar' // Default to Arabic
    ) {
        $apiKey = BusinessSetting::where("key","map_api_key")->first()->value; // Use environment variable for security

        // Format the lat,lng strings for origin and destination
        ...
        $response = Http::get($url, [
            ...
            'mode' => $mode, // Travel mode: driving, walking, bicycling, or transit
            'language' => $language, // Set the response language
            'key' => $apiKey,
        ]);
        ...
        if ($response->successful() && isset($data['rows'][0]['elements'][0]['duration']['value'])) {
            // Extract travel time in seconds
            $travelTimeSeconds = $data['rows'][0]['elements'][0]['duration']['value'];

            // Convert travel time to minutes
            $travelTimeMinutes = ceil($travelTimeSeconds / 60);

            // Add travel time and preparation time to current time
            $currentTime = Carbon::now();

            $minDeliveryTime = $currentTime
                ->copy()
                ->addMinutes($travelTimeMinutes + $storeMinTime);

            $maxDeliveryTime = $currentTime
                ->copy()
                ->addMinutes($travelTimeMinutes + $storeMaxTime);

            return [
                'travel_time' => "{$travelTimeMinutes} دقيقة", // Travel time in Arabic
                'min_delivery_time' => $minDeliveryTime->format('g:i A'), // Min delivery time
                'max_delivery_time' => $maxDeliveryTime->format('g:i A'), // Max delivery time
            ];
        }

        return [$origin,$destination];
    }
    function getFileExtension(UploadedFile $uploadedFile)
    {
        ...
        return $extensions[$mimeType] ?? 'bin'; // 'bin' as a fallback for unknown types
    }
    ...
    public function place_order(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'payment_method' => 'required|in:cash_on_delivery,digital_payment,wallet,offline_payment,package',
        ...
        if ($request->order_type !== 'parcel') {
            ...
            if ($request->distance > (float)BusinessSetting::where(['key' => 'limit_for_delevry'])->first()->value) {
                $distance_after_sub = $request->distance - (float)BusinessSetting::where(['key' => 'limit_for_delevry'])->first()->value;
                $price_after_add = $distance_after_sub * (float)BusinessSetting::where(['key' => 'price_after_limit'])->first()->value;
                $original_delivery_charge = $original_delivery_charge + $price_after_add;
            }
            ...
            if ($request->distance > (float)BusinessSetting::where(['key' => 'limit_for_delevry'])->first()->value) {
                $distance_after_sub = $request->distance - (float)BusinessSetting::where(['key' => 'limit_for_delevry'])->first()->value;
                $price_after_add = $distance_after_sub * (float)BusinessSetting::where(['key' => 'price_after_limit'])->first()->value;
                $delivery_charge = $delivery_charge + $price_after_add;
            }
            ...
            if (!empty($request->file('order_attachment'))) {
                ...
                foreach ($request->order_attachment as $img) {
                    $extension = $this->getFileExtension($img);
                    $image_name = Helpers::upload('order/', $extension, $img);
                    if (in_array($img->extension(), ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp'])) {
                        array_push($img_names, ['img' => $image_name, 'storage' => Helpers::getDisk()]);
                    } else {
                        array_push($img_names, ['vid' => $image_name, 'storage' => Helpers::getDisk()]);
                    }
                }
                $images = $img_names;
            }
        ...
        if ($user) {
            if ($request->payment_method == "wallet") {
                ...
                if ($user->is_2ydha) {
                    Log::info("this is 2ydha if 1");

                    if ($user->active_2ydha == false) {
                        Log::info("this is 2ydha if 2");

                        if ($user->amount_2ydha <= $user->wallet_balance) {
                            Log::info("this is 2ydha if 3");

                            $user->active_2ydha = true;
                            $user->save();
                        } else {
                            Log::info("this is 2ydha error  3");

                            return response()->json(['status' => false, 'message' => 'يجب عليك دفع ' . $user->amount_2ydha - $user->wallet_balance . " ريال" . " ", 'data' => $user->amount_2ydha - $user->wallet_balance], 400);
        ...
        $order->price_services = $request['price_services'];
        ...
        if ($request->header('moduleId')==3)
        {
            $carts = Cart::where('user_id', $order->user_id)->where('is_guest', 0)->where('module_id', $request->header('moduleId'))
            ...
            $totalPricePanda=0;
            $totalPriceCarfour=0;
            log::info("cart",["carts"=>$carts]);
            forEach($carts as $items){
                $product = Item::with('module')->active()->find($items['item_id']);
                log::info("product",["product"=>$product]);

                if($product->carfour_id!=null&&$product->panda_id!=null){

                    $priceCarfour= $this->getItemCarfourPrice($product->carfour_id);

$pandaPrice=$this->getItempandaPrice($product->panda_id);
                    log::info("panda",["total"=>$pandaPrice]);
                    log::info("carfour",["total"=>$priceCarfour]);

                    $totalPricePanda=$totalPricePanda+$pandaPrice;
                    $totalPriceCarfour=$totalPriceCarfour+$priceCarfour;
                }

            }
            if($totalPriceCarfour<$totalPricePanda){
                $order->store_id = 210;
                $order->admin_earn=($request->order_amount-round($delivery_charge, config('round_up_to_digit')) )-$totalPriceCarfour;

            }else{
                $order->store_id = 3;
                log::info("this is 2ydha if 4",["total"=>$totalPricePanda,"orderPrice"=>$request->order_amount]);
                $order->admin_earn=($request->order_amount-round($delivery_charge, config('round_up_to_digit')) )-$totalPricePanda;
            }
        ...
        $order->collection_order = isset($request->collection_order) ? $request->collection_order : false;

        $order->is_guest = $request->user ? 0 : 1;
        $order->otp = rand(1000, 9999);
        if($request->order_type=="delivery"){
            $order->otp_store = rand(1000, 9999);
        }
        ...
        if ($request->order_type !== 'parcel') {
            if (!empty($request->file('order_instraction_attachemnet'))) {
                $img_names_instraction = [];
                $images_instraction = [];
                foreach ($request->order_instraction_attachemnet as $img) {
                    $extension = $this->getFileExtension($img);
                    $img_names = Helpers::upload('order/', $extension, $img);
                    if (in_array($img->extension(), ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp'])) {
                        array_push($img_names_instraction, ['img' => $img_names, 'storage' => Helpers::getDisk()]);
                    } else {

                        if (!in_array($img->extension(), ['mp3', 'wav', 'ogg',])) {
                            array_push($img_names_instraction, ['vid' => $img_names, 'storage' => Helpers::getDisk()]);
                        } else {
                            array_push($img_names_instraction, ['audio' => $img_names, 'storage' => Helpers::getDisk()]);

                        }
                    }
                }
                $images_instraction = $img_names_instraction;
                $order->order_instraction_attachemnet = json_encode($images_instraction);
        ...
        if ($additional_charge_status == 1&&$request->payment_method=="cash_on_delivery") {
        ...
        if ($request->order_type !== 'parcel') {
            foreach ($carts as $c) {
                if ($c['item_type'] === 'App\Models\ItemCampaign' || $c['item_type'] === 'AppModelsItemCampaign') {
                    ...
                    if ($product) {
                        if ($request->header('moduleId')==3){
                            $order->admin_earn=$order->admin_earn+$product->organal_price;
                        }else{
                            if ($product->store_id != $order->store_id) {
                                return response()->json([
                                    'errors' => [
        ...
        if ($request->user && $request->payment_method == "package") {
            if ($request->user->km !== null && $distance_data <= $request->user->km) {
                $customer = $request->user;
                $customer->zone_id = $order->zone_id;
                $customer->km = $customer->km - $distance_data;
                ...
                return response()->json([
                    "message" => "you may don't subscribe in package or Your package is not enough,please subscribe to new package or change payment method",
                ], 400);
            }
        ...
        try {
            ...
            $order->save();
            if ($request->collection_order == true) {

                $this->addOrderToGroupTable($request, $id);
            }
            if ($request->order_type !== 'parcel') {
            ...
            CheckOrderAcceptance::dispatch($order);
    ...
    public function addOrderToGroupTable(Request $request, $id)
    {
        $TableOrder = new order_groups();
        $TableOrder->main_order_id = isset($request->order_group_id) && !is_null($request->order_group_id) ? $request->order_group_id : $id;
        $TableOrder->order_group_id = $id;
        if ($TableOrder->main_order_id != $id) {
            $TableOrder->save();
    ...
    protected function getItemPandaPrice($pandaProductsId)
    {
        $client = new Client([
            'base_uri' => 'https://api.panda.sa/v3/',
            'timeout' => 50.0,
        ]);
        $response = $client->request('GET', 'products/' . $pandaProductsId, [
            'headers' => [
        ...
        if (isset($data['data'])) {
            return $data['data']['product']["varieties"][0]["price"];
        } else {
            return "notFound";
    ...
    protected function getItemCarfourPrice($carfourProductsId)
    {
        $client = new Client([
            'base_uri' => 'https://www.carrefourksa.com/api/v4/relevance/',
            'timeout' => 50.0,
        ]);
        $response = $client->request('GET', 'products/' . $carfourProductsId, [
            'query' => [
                "lang" => "ar",
                "placements" => "personal_page.echo_seed|item_page.frequently_bought_together_mobile",
                "displayCurr" => "SAR",
                "latitude" => "24.7136",
                "longitude" => "46.6753"
            ],
        ...
        $data = json_decode($response->getBody()->getContents(), true);
        if (isset($data['data']['placements'][0]["recommendedProducts"])) {
            return $data['data']['placements'][0]["recommendedProducts"][0]["price"]["price"];
        } else {
            return "notFound";
    ...
    public function store_request_services_parcel(Request $request)
    {
        ...
        $requests->delivery_man_id = $request->delivery_man_id;
        $requests->order_id = $request->order_id;
        $requests->save();
        $dataCustomer = [
            'title' => "هناك عرض جديد لطلبك رقم ",
            'description' => "عرص جديد لطلبك",
            'order_id' => $order->id,
            'image' => '',
        ];
        $dataDeliveryMan = [
            'title' => "تم تقديم عرضك",
            'description' => "تم تقديم عرضك",
            'order_id' => $order->id,
            'image' => '',
        ];
        /*       Helpers::send_push_notif_to_device($order->customer->fcm_token,$dataCustomer);
               Helpers::send_push_notif_to_device($order->delivery_man->fcm_token,$dataDeliveryMan);*/
        return response()->json([
            "status" => "success", "message" => "add price of service successfully"], 200);


    }

    public function accept_request_services_parcel(Request $request)
    {
        $validated = Validator::make($request->all(), [
            'request_id' => 'required',
        ]);
        ...
        $requests = \App\Models\request::with('order')->find($request->request_id);

        $order = $requests->order;
        $order->order_amount = $requests->price;
        $order->price_services = $requests->price;
        $order->delivery_charge = $requests->price;

        $order->delivery_man_id = $requests->delivery_man_id;

        $order->save();
        /* $dataCustomer = [
             'title' => $order->customer->name."تهانينا!!",
             'description' => " سيتواصل معك الفني الان لترتيب العمل",
             'order_id' => $order->id,
             'image' => '',
         ];
         $dataDeliveryMan = [
             'title' => $order->id."تم قبول عرضك",
             'description' => "تم قبول عرضك",
             'order_id' => $order->id,
             'image' => '',
         ];
         Helpers::send_push_notif_to_device($order->customer->fcm_token,$dataCustomer);
         Helpers::send_push_notif_to_device($order->delivery_man->fcm_token,$dataDeliveryMan);*/
        return response()->json([
            "status" => "success", "message" => "accept the request successfully",], 200);
    ...
    public function get_order_list(Request $request)
    {
        ...
        $paginator = Order::with(['store', 'delivery_man.rating', 'parcel_category', 'refund:order_id,admin_note,customer_note', "OrderRequests", 'OrderRequests.deliveryMan'])->withCount('details')->where(['user_id' => $user_id])->whereIn('order_status', ['delivered', 'canceled', 'refund_requested', 'refund_request_canceled', 'refunded', 'failed'])
    ...
    public function get_running_orders(Request $request)
    {
        ...
        $paginator = Order::with(['store', 'delivery_man.rating', 'parcel_category', "OrderRequests", 'OrderRequests.deliveryMan', 'group_order'])
            ->when(!isset($request->user), function ($query) {
                $query->where('is_guest', 1);
            })
            ->when(isset($request->user), function ($query) {
        ...
        $orders = array_map(function ($data) {
            ...
            $data['store'] = $data['store'] ? Helpers::store_data_formatting($data['store']) : $data['store'];
            $data['delivery_man'] = $data['delivery_man'] ? Helpers::deliverymen_data_formatting([$data['delivery_man']]) : $data['delivery_man'];
            if ($data["collection_order"] && $data->group_order) {
                $data["collections_order"] = array_map(function ($data) {
                    return $data["order_group_id"];
                }, $data->group_order->toArray());
            }
            return $data;
        }, $paginator->items());
        $data = [
    ...
    public function get_order_details(Request $request)
    {
        ...
        $order = Order::with('details', 'offline_payments', 'parcel_category', 'OrderRequests',
            'OrderRequests.deliveryMan', "group_order.OrderGroupId.details",)
            ->when(!isset($request->user), function ($query) {
                $query->where('is_guest', 1);
            })
            ->when(isset($request->user), function ($query) {
        ...
        $orderGroup = isset($order->group_order) ? $order->group_order : null;

//        dd( $order->group_order->first()->OrderGroupId->details);

        if ($details != null && $details->count() > 0) {
            $details = Helpers::order_details_data_formatting($details, $orderGroup);
