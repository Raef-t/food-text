...
public  function requestExchangeWalletMoney ( Request $request )
{
    $validator = Validator::make($request->all(), [
        'userReceiver' => 'required',
    ]);
    if ($validator->fails()) {
        return response()->json(['errors' => Helpers::error_processor($validator)], 403);
    }
    $userReceiver=User::where(['phone' => $request->userReceiver])->first();
    if($userReceiver){
    $user=User::where(['id' => $request->user()->id])->first();
    $otpUser= rand(1000, 9999);
    $otpUserReceiver= rand(1000, 9999);
    $userReceiver->otp_wallet=$otpUserReceiver;
    $userReceiver->save();
    $user->otp_wallet=$otpUser;
    $user->save();
    $data = [
        'title' => translate('طلب تحويل من المحفظه'),
        'description' => "كود التحويل الخاص بك هوا ".$otpUser,
        'type' => 'new_order',
        "image"=>"",
        "order_id"=>1,
        "module_id"=>3,
...
"conversation_id"=>0
    ...
    $dataReceiver = [
        'title' => translate('طلب تحويل الي المحفظه'),
        'description' => "كود التحويل الخاص بك هوا ".$otpUserReceiver,
        'type' => 'new_order',
        "image"=>"",
        "order_id"=>1,
        "module_id"=>3,
    ...
    /*    'order_id' => $order->id,
                    ...
                    'type' => 'new_order',*/
   $notfi= Helpers::send_push_notif_to_device($request->user()->cm_firebase_token,$data);
        $notfiuser=Helpers::send_push_notif_to_device($userReceiver->cm_firebase_token,$dataReceiver);
    log::info('send push notif to userReceiver '.$userReceiver->id.' '.$userReceiver->cm_firebase_token);
    log::info('send push notif to userReceiver '.$request->user()->id.' '.$request->user()->cm_firebase_token);
    return response()->json(['message' => translate('messages.successfully_updated'),"notfi"=>$notfi,"notfiuser"=>$notfiuser], 200);
    }else{
        return response()->json(['errors'=>[['code'=>'userReceiver', 'message'=>translate('messages.user_not_found')]]],203);
    }
}
public  function ExchangeWalletMoney ( Request $request )
{
    $validator = Validator::make($request->all(), [
        "otpUser"=> "required",
        "otpReceiver"=> "required",
        "walletMoney"=> "required",
        'userReceiver' => 'required',
    ]);
    if ($validator->fails()) {
        return response()->json(['errors' => Helpers::error_processor($validator)], 403);
    }
    if($request->user()->wallet_balance<$request->walletMoney){
        return response()->json(['errors'=>[['code'=>'wallet_balance', 'message'=>translate('messages.not_enough_money_in_your_wallet')]]],203);
    }
if($request->user()->otp_wallet!=$request->otpUser){
    return response()->json(['errors'=>[['code'=>'otp_wallet', 'message'=>translate('messages.invalid_otp')]]],203);
}
    $userReceiver=User::where(['phone' => $request->userReceiver])->firstOrFail();
    $user=User::where(['id' => $request->user()->id])->first();

if($userReceiver->otp_wallet!=$request->otpReceiver){
    return response()->json(['errors'=>[['code'=>'otp_wallet', 'message'=>translate('messages.invalid_otp_user')]]],203);
}
$userReceiver->wallet_balance=$userReceiver->wallet_balance+$request->walletMoney;
$userReceiver->save();
    $user->wallet_balance=  $user->wallet_balance-$request->walletMoney;
    $user->save();

    $data = [
        'title' => translate('تم تحويل المبلغ بنجاح'),
        'description' => $userReceiver->l_name." ".$userReceiver->f_name."الي".$request->walletMoney."تم تحويل مبلغ قيمته",
        "image"=>"",
        "type"=>"new_order",
        "order_id"=>1,
        "module_id"=>3,
    ...
    $dataReceiver = [
        'title' => translate('تم التحويل الي المحفظه'),
        'description' => $request->user()->l_name." ".$request->user()->f_name."من".$request->walletMoney."تم تحويل مبلغ قيمته",
        "image"=>"",
        "type"=>"new_order",
        "order_id"=>1,
        "module_id"=>3,
    ...
    Helpers::send_push_notif_to_device($request->user()->cm_firebase_token,$data);
    Helpers::send_push_notif_to_device($userReceiver->cm_firebase_token,$dataReceiver);
