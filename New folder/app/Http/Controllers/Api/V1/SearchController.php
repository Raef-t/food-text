...
//        $key = explode(' ', $request['name']);
        ...
        ->where(function ($q) use ($key) {
            $text = preg_replace('/[ًٌٍَُِّْ]/u', '', $key);
            // Normalize Alif variations
            $text = str_replace(['أ', 'إ', 'آ'], 'ا', $key);
           // $q->orWhereRaw("MATCH(name) AGAINST(? IN NATURAL LANGUAGE MODE)", [$text]);

            $q->orWhere('name', 'like', "%{$text}%");

            /*foreach ($key as $value) {
            }*/
            $q->orWhereHas('translations',function($query)use($key,$text){
                $query->where(function($q)use($key,$text){
                    $q->where('value', 'like', "%{$text}%");
                   // $q->whereRaw("MATCH(value) AGAINST(? IN BOOLEAN MODE)", [$text . '*']);
