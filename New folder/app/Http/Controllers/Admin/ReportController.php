...
class ReportController extends Controller
{
    ...
    function calculateDeliveryTime(
        ...
        $createdAt,  // Store maximum preparation time (in minutes)
        $mode = 'driving',
    ...
    public function day_wise_report(Request $request)
    {
        ...
        $order_transactions = array_map(function($order) {
            $orderStore = Helpers::store_data_formatting($order->order->store);

            $deliveryAddress = json_decode($order->order->delivery_address, true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                $deliveryAddress = []; // Fallback to an empty array in case of JSON decoding error
            }

            $storeLat = $orderStore->latitude ?? 0;
            $storeLng = $orderStore->longitude ?? 0;
            $destLat = $deliveryAddress["latitude"] ?? 0;
            $destLng = $deliveryAddress["longitude"] ?? 0;

            $storeMinTime = isset($orderStore->delivery_time) ? (int)explode('-', $orderStore->delivery_time)[0] : 0;
            $storeMaxTime = isset($orderStore->delivery_time) ? (int)explode('-', $orderStore->delivery_time)[1] : 0;

            $createdAt = $order->created_at instanceof \Carbon\Carbon ? $order->created_at : \Carbon\Carbon::parse($order->created_at);

            $time = $this->calculateDeliveryTime(
                $storeLat,
                $storeLng,
                $destLat,
                $destLng,
            ...
            $order->min_time = $time["min_delivery_time"];
            $order->max_time = $time["max_delivery_time"];

            return $order;
        }, $order_transactions->items());
    ...
    public function item_percent_report(Request $request)
    {
        $key = explode(' ', $request['search']);
        if (session()->has('from_date') == false) {
            session()->put('from_date', now()->firstOfMonth()->format('Y-m-d'));
        ...
        $store = is_numeric($store_id) ? Store::findOrFail($store_id) : null;
        $category = is_numeric($category_id) ? Category::findOrFail($category_id) : null;
        $items = \App\Models\Item::withoutGlobalScope(StoreScope::class)->where("organal_price","!=",null)
            ->withCount([
                'orders' => function ($query) use ($from, $to, $filter) {
                    $query->when(isset($from) && isset($to) && $from != null && $to != null && $filter == 'custom', function ($query) use ($from, $to) {
                        return $query->whereBetween('created_at', [$from . " 00:00:00", $to . " 23:59:59"]);
...
foreach ($items as $item) {
    if ($item->organal_price === $item->price) {
        $item->organal_price=null;
        $item->save();
    }
}
        return view('admin-views.report.item-report_percent', compact('zone', 'store', 'category', 'items', 'filter'));
    }
