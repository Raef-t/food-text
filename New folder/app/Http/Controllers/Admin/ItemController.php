...
"));
        ...
        if($request->youtube!=null&&$request->youtube_index!=null){
            $item->youtube=$request->youtube;
            $item->youtube_index=$request->youtube_index;
        }
        $item->store_id = $request->store_id;
    ...
    public function recommended(Request $request): \Illuminate\Http\RedirectResponse
    {
        $product = Item::withoutGlobalScope(StoreScope::class)->findOrFail($request->id);
    ...
    {
        $products=CategoryLogic::productsAdmin($categoryId,"[1]");
        return response()->json($products);

    }
  public  function addPriceToProducts(Request $request){
      $validator = Validator::make($request->all(), [
          'percent' => 'required|numeric|between:.01,100,00',
          'min_Price' => 'required|numeric|between:.01,100,00',
          'max_price' => 'required|numeric|between:.01,100,00',
      ],);

     $products=CategoryLogic::productsAdmin($request->category_id,"[1]");
      $count=0;
      foreach ($products as $product){

         if($product->price>=$request->min_Price){
              if($product->price<=$request->max_price){

                  $product->organal_price = $product->price;
                  $product->percent_added = $request->percent;
                  $product->price = ($product->price * ($request->percent / 100)) + $product->price;
                  $product->is_addet = true;
                  $product->save();
                  $count++;
              }
          }
      }
      Toastr::success(":$count " .translate('messages.product_added_successfully'));
      return back();
  }
  public  function removePriceToProducts(Request $request){
      $validator = Validator::make($request->all(), [
          'min_Price' => 'required|numeric|between:.01,100,00',
          'max_price' => 'required|numeric|between:.01,100,00',
      ],);
      ...
      foreach ($products as $product){
          if($product->price>=$request->min_Price){
              if($product->price<=$request->max_price){
                  $product->price=$product->organal_price;
                  $product->is_addet=false;
                  $product->save();
                  $count++;
              }
          }
