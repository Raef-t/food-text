...
class VendorController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $vendor->email = $request->phone . "@shellafood.com";
        ...
        $store->email = $request->phone . "@shellafood.com";
    ...
    public function view(Request $request, $store_id, $tab = null, $sub_tab = 'cash')
    {
        ...
        $categories = $tab == 'settings' ? CollectionGroup::all() : Category::where([
            'position' => 0,
            'module_id' => $store->module_id
        ])->with("childes")->get();
       
        if ($tab == 'settings') {
            return view('admin-views.vendor.view.settings', compact('store', 'categories'));
        } else if ($tab == "price") {
            return view('admin-views.vendor.view.price', compact('store', 'categories'));
        } else if ($tab == 'order') {
    ...
    public function addCategoryToStore($id, Store $store)
    {
        $categoryIds = json_decode($id, true);
        /*  $categoryId=(int)$id;
        ...
        $store->category_idsss = $categoryIds;
        $store->save();
    ...
    public function busy(Store $store, Request $request)
    {
        $store->busy = $request->status;
        $store->save();
        $vendor = $store->vendor;
