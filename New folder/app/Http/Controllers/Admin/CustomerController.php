...
class CustomerController extends Controller
{
    ...
    public function customer_list(Request $request)
    {
        ...
        })->when(isset($filter) && $filter == '2ydha' , function ($query) {
            $query->where('is_2ydha' ,1);
        $query->where('active_2ydha' ,0);
    })
...
public  function UpdateAydhaUser(Request $request, user $user)
{
$user->is_2ydha=$request->is_2ydha;
if($request->is_2ydha==0){
    $user->active_2ydha=0;
}
$user->save();
Toastr::success(translate('messages.customer') . translate('messages.status_updated'));
...
public  function activeAydhaUser(Request $request, user $user)
{
    $user->active_2ydha=$request->active_2ydha;
$user->save();
Toastr::success(translate('messages.customer') . translate('messages.status_updated'));
return back();
}
public  function amountAydhaUser(Request $request, user $user)
{
    $user->amount_2ydha=$request->amount_2ydha;
$user->save();
Toastr::success(translate('messages.customer') . translate('messages.status_updated'));
return back();
}
