                        ...
                        <div class="d-flex flex-wrap gap-3">
                            <div class="color-card flex-column align-items-center justify-content-center color-7 flex-grow-1">
                                ...
                                <div class="d-flex flex-column align-items-center">
                                    ...
                                    <div class="subtitle">
                                        {{$customer->is_2ydha?translate('messages.aydha_balance'):translate('messages.wallet_balance')}}
                                    </div>
                                </div>
                            </div>
            ...
            <div class ='row g-3'>
                <td>
                    <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox{{$customer->id}}">
                        <input type="checkbox" data-url="{{route('admin.UpdateAydhaUser',["user"=>$customer['id'],"is_2ydha"=>$customer->is_2ydha?0:1])}}" class="toggle-switch-input redirect-url" id="stocksCheckbox{{$customer->id}}" {{$customer->is_2ydha?'checked':''}}>
                ...
                <br>
                <h3>
                    هل العميل مشترك ف خدمه قيدها
                </h3>
            </div>
            <div class ='row g-3'>
                <td>
                    <label class="toggle-switch toggle-switch-sm" for="stocksCheckboxactive{{$customer->id}}">
                        <input type="checkbox" data-url="{{route('admin.activeAydhaUser',["user"=>$customer['id'],"active_2ydha"=>$customer->active_2ydha?0:1])}}" class="toggle-switch-input redirect-url" id="stocksCheckboxactive{{$customer->id}}" {{$customer->active_2ydha?'checked':''}}>
                        <span class="toggle-switch-label mx-auto">
                                            <span class="toggle-switch-indicator"></span>
                                        </span>
                    </label>
                ...
                <br>
                <h3>
                    هل العميل مفعل له خدمه قيدها
                </h3>
            </div>
                <form method="post" action="{{ route('admin.amountAydhaUser',["user"=>$customer,]) }}">
@csrf
                    <div class="form-group mb-2">
                        <label class="form-label text-capitalize"
                               for="amount_2ydha">حد المشتريات لخدمه قيدها </label>
                        <input type="number" name="amount_2ydha" class="form-control"
                               id="amount_2ydha" min="0" step=".01"  placeholder="{{ translate('messages.Ex:_100') }}"
                               value="{{ $customer->amount_2ydha }}"
                               required>
                    </div>
                    <button type="{{ 'submit' }}"
                            class="btn btn--primary mb-3 mt-3">{{ translate('submit') }}</button>
                </form>
                </div>
        </div>
                    ...
                    <div class="card-header">
                        <h4 class="card-title d-flex flex-wrap align-items-center gap-2">
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <span class=""> {{ translate('aydha_info_user') }}</span>
                            </div>
                            <span class="badge badge-soft-info">{{ translate('name') }}: {{ $customer->AydhaUser->f_name }} {{ $customer->AydhaUser->s_name }} {{ $customer->AydhaUser->t_name }} {{ $customer->AydhaUser->l_name }}</span>
                        </h4>
                    </div>
                    <!-- End Header -->
                        ...
                        <div class="card-body">
                            <div class="media gap-3 flex-wrap">
                                ...
                                <div class="media-body">
                                    <div class="key-value-list d-flex flex-column gap-2 text-dark" style="--min-width: 60px">
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('birth_date') }}</div>:
                                            <div class="font-semibold">{{$customer->AydhaUser->birth_date}}</div>
                                        </div>
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('status') }}</div>:
                                            <a  class="text-dark font-semibold">{{$customer->AydhaUser->status}}</a>
                                        </div>
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('count_family') }}</div>:
                                            <a  class="text-dark font-semibold">{{$customer->AydhaUser['count_family_members']}}</a>
                                        </div>
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('number_of_verification') }}</div>:
                                            <a  class="text-dark font-semibold">{{$customer->AydhaUser['number_of_verification']}}</a>
                                        </div>
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('startDate_of_verification') }}</div>:
                                            <a  class="text-dark font-semibold">{{$customer->AydhaUser['startDate_of_verification']}}</a>
                                        </div>
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('endDate_of_verification') }}</div>:
                                            <a  class="text-dark font-semibold">{{$customer->AydhaUser['endDate_of_verification']}}</a>
                                        </div>
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('whatsApp_number') }}</div>:
                                            <a  class="text-dark font-semibold">{{$customer->AydhaUser['whatsApp_number']}}</a>
                                        </div>
                                    </div>
                    ...
                    <div class="card-header">
                        <h4 class="card-title d-flex flex-wrap align-items-center gap-2">
                            <div class="d-flex align-items-center gap-1">
                                ...
                                <span class=""> {{ translate('home_info') }}</span>
                            </div>
                            <span class="badge badge-soft-info">{{ translate('location_of_home') }}: {{ $customer->AydhaUser->location_of_home}}</span>
                        </h4>
                    </div>
                    <!-- End Header -->
                        ...
                        <div class="card-body">
                            <div class="media gap-3 flex-wrap">
                                ...
                                <div class="media-body">
                                    <div class="key-value-list d-flex flex-column gap-2 text-dark" style="--min-width: 60px">
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('type_of_house') }}</div>:
                                            <div class="font-semibold">{{$customer->AydhaUser->type_of_house}}</div>
                                        </div>
                                        <div class="key-val-list-item d-flex gap-3">
                                            <div>{{ translate('status_of_house') }}</div>:
                                            <a  class="text-dark font-semibold">{{$customer->AydhaUser['status_of_house']}}</a>
                                        </div>
                                        @if($customer->AydhaUser['status_of_house']=="ايجار")
                                            <div class="key-val-list-item d-flex gap-3">
                                                <div>{{ translate('price_of_rent') }}</div>:
                                                <a  class="text-dark font-semibold">{{$customer->AydhaUser['price_of_rent']}}</a>
                                            </div>
                        ...
                        <div class="card-header">
                            <h4 class="card-title d-flex flex-wrap align-items-center gap-2">
                                ...
                                <span class="card-header-icon">
                                    ...
                                    <span class=""> {{ translate('work_info') }}</span>
                                </div>
                                <span class="badge badge-soft-info">{{ translate('type_of_work') }}: {{ $customer->AydhaUser->type_of_work}}</span>
                            </h4>
                        </div>
                        <!-- End Header -->
                            ...
                            <div class="card-body">
                                <div class="media gap-3 flex-wrap">
                                    ...
                                    <div class="media-body">
                                        <div class="key-value-list d-flex flex-column gap-2 text-dark" style="--min-width: 60px">
                                            <div class="key-val-list-item d-flex gap-3">
                                                <div>{{ translate('title_of_work') }}</div>:
                                                <div class="font-semibold">{{$customer->AydhaUser->title_of_work}}</div>
                                            </div>
                                            <div class="key-val-list-item d-flex gap-3">
                                                <div>{{ translate('years_of_work') }}</div>:
                                                <a  class="text-dark font-semibold">{{$customer->AydhaUser['years_of_work']}}</a>
                                            </div>
                                                <div class="key-val-list-item d-flex gap-3">
                                                    <div>{{ translate('salary_of_work') }}</div>:
                                                    <a  class="text-dark font-semibold">{{$customer->AydhaUser['salary_of_work']}}</a>
                                                </div>
                        ...
                        <div class="card-header">
                            <h4 class="card-title d-flex flex-wrap align-items-center gap-2">
                                ...
                                <span class="card-header-icon">
                                    ...
                                    <span class=""> {{ translate('Docs') }}</span>
                                </div>
                                <span class="badge badge-soft-info">{{ translate('Docs') }}</span>
                            </h4>
                        </div>
                        <!-- End Header -->

                        <!-- Body -->
                        @if($customer->AydhaUser->photo_of_verification&&count(json_decode($customer->AydhaUser->photo_of_verification))!=0)
                            @foreach(json_decode($customer->AydhaUser->photo_of_verification) as $photo)
                                <div class="card-body">
                                    <div class="media gap-3 flex-wrap">
                                         <a href="{{ asset('storage/app/public/aydha_user/') . '/' . $photo }}" target="_blank"><img class="avatar-img onerror-image" width="400" height="200" data-onerror-image="{{asset('public/assets/admin/img/160x160/img1.jpg')}}" src="{{asset('storage/app/public/aydha_user/')."/".$photo }}"
                                             alt="Image Description">
                                         </a>
                            ...
                            @endforeach
                                ...
                                @endif
                                <hr>
                                @if($customer->AydhaUser->photo_of_work&&count(json_decode($customer->AydhaUser->photo_of_work))!=0)
                                    @foreach(json_decode($customer->AydhaUser->photo_of_work) as $photo)
                                <div class="card-body">
                                    <div class="media gap-3 flex-wrap">
                                        <a href="{{ asset('storage/app/public/aydha_user/') . '/' . $photo }}" target="_blank"><img class="avatar-img onerror-image" width="400" height="200" data-onerror-image="{{asset('public/assets/admin/img/160x160/img1.jpg')}}" src="{{asset('storage/app/public/aydha_user/')."/".$photo }}"
                                                                                                                                    alt="Image Description">
                            ...
                            @endforeach
                                        ...
                                        @if($customer->AydhaUser->photo_of_home&&count(json_decode($customer->AydhaUser->photo_of_home))!=0)
                                            @foreach(json_decode($customer->AydhaUser->photo_of_home) as $photo)
                                <div class="card-body">
                                    <div class="media gap-3 flex-wrap">
                                        <a href="{{ asset('storage/app/public/aydha_user/') . '/' . $photo }}" target="_blank"><img class="avatar-img onerror-image" width="400" height="200" data-onerror-image="{{asset('public/assets/admin/img/160x160/img1.jpg')}}" src="{{asset('storage/app/public/aydha_user/')."/".$photo }}"
                                                                                                                                    alt="Image Description">
