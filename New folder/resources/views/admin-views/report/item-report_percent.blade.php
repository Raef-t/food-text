            ...
            <div class="table-responsive datatable-custom" id="table-div">
                <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap card-table"
                    ...
                    <tr>
                        ...
                        <th>{{translate('messages.original_price')}}</th>
                        <th>{{translate('messages.percent')}}</th>
                    ...
                    @foreach($items as $key=>$item)
                        <tr>
                            ...
                            <td>
                                {{ \App\CentralLogics\Helpers::format_currency($item->organal_price) }}
                            </td>
                            <td>
                                {{ $item->percent_added ?? ($item->organal_price != 0 ? round(((($item->price - $item->organal_price) / $item->organal_price) * 100)) : 0) }} %                            <td>
...
@push('script_2')
    ...
    <script>
        ...
        $(document).on('ready', function () {
            ...
            $('#category_id').select2({
                ajax: {
                    url: '{{ url('/') }}/admin/item/get-categories?parent_id=0&sub_category=1',
                    data: function(params) {
                        return {
                            q: params.term, // search term
                            @if(request('module_id'))module_id: {{request('module_id')}}, @endif
                    ...
                    processResults: function(data) {
                        // Format results to display category name followed by ID in parenthesis
