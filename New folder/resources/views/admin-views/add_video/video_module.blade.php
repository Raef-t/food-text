...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{route('admin.video.add_video_Module',)}}" method="post">
                    @csrf
                    <div class="row">
                        <div class="col-md-8">
                            <div class="form-group col-sm-6">
                                <label class="input-label"
                                       for="exampleFormControlSelect1">{{translate('messages.select-module')}}
                                    <span class="input-label-secondary">*</span></label>
                                <select id="exampleFormControlSelect1" name="store_id" class="form-control js-select2-custom" required>
                                    <option value="" selected disabled>{{translate('select-module')}}</option>
                                    @foreach($stores as $store)
                                        <option value="{{$store['id']}}" >{{$store['module_name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-sm-6">
                ...
                <div class="table-responsive datatable-custom">
                    <table id="columnSearchDatatable"
                        ...
                        <tr>
                            ...
                            <th class="border-0 w--1">{{translate('messages.moduleID')}}</th>
                        ...
                        @foreach($videos as $key=>$category)
                            <tr>
                                ...
                                <td>
                                    <span class="d-block font-size-sm text-body">
                                        {{Str::limit($category['module_id'], 20,'...')}}
                                    </span>
                                </td>
                                <td>
