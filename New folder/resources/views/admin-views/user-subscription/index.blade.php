                ...
                <div class="col-xl-10 col-md-9 col-sm-8 mb-3 mb-sm-0 {{$parcel_order ? 'mb-2':''}}">
                    <h1 class="page-header-title text-capitalize m-0">
                        ...
                        <span>
                            ...
                            <span class="badge badge-soft-dark ml-2">2</span>
                        </span>
        ...
        <div class="table-responsive datatable-custom">
            <table id="datatable"
                ...
                <tr>
                    ...
                    <th class="border-0">{{translate('messages.package')}}</th>
                ...
                @foreach($Subscriptions as $key=>$order)
                    ...
                    <tr class="status-{{$order->payment_status}} class-all">

                        <td class="table-column-pl-0">
                            <a href="{{route('admin.parcel.subscription.showInfo',['Subscriptions'=>$order['id']])}}">  #  {{$order['id']}}</a>
                        </td>
                        <td>
                            <div>
                                <div>
                        ...
                        <td>
                            ...
                            @elseif($order->user)
                                ...
                                <a class="text-body" href="{{route('admin.customer.view',[$order['user_id']])}}">
                                    <strong>
                                        <div> {{$order->user['f_name'].' '.$order->user['l_name']}}</div>
                                    </strong>
                                </a>
                        ...
                        <td>
                            ...
                            @elseif ($order->package->name)
                                <div><a class="text--title" href="{{route('admin.parcel.index',)}}"
                                        alt="view store">{{$order->package->name}}</a></div>
                            @else
                        ...
                        <td>
                            <div class="text-right mw--85px">
                                <div>
                                    {{\App\CentralLogics\Helpers::format_currency($order->package->price)}}
                                </div>
                        ...
                        <td>
                            <div class="btn--container justify-content-center">
                                <a class="ml-2 btn btn-sm btn--warning btn-outline-warning action-btn"
                                   href="{{route('admin.parcel.subscription.showInfo',['Subscriptions'=>$order['id']])}}">
...
@push('script_2')
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        action: function (e, dt, node, config) {
                            window.location.href = '{{route("admin.order.export",['status'=>"all",'file_type'=>'excel','type'=>$parcel_order?'parcel':'order', request()->getQueryString()])}}';
                        }
                    },
                    {
                        extend: 'csv',
                        className: 'd-none',
                        action: function (e, dt, node, config) {
                            window.location.href = '{{route("admin.order.export",['status'=>"all",'file_type'=>'csv','type'=>$parcel_order?'parcel':'order', request()->getQueryString()])}}';
                        }
                    },
                    // {
                    //     extend: 'pdf',
    ...
    <script type="text/javascript">
        ...
        // Poll the server every 1 min
        setInterval(checkForNewOrders, 60000); // 60000 milliseconds = 1 min
    </script>
